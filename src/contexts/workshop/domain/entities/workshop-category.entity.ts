...
export class WorkshopCategory {
  ...
  static create(name: string, description?: string): WorkshopCategory {
    ...
    const now = new Date();
    return new WorkshopCategory(WorkshopCategoryId.generate(), name, description ?? null, now, now);
  }
  ...
  update(name?: string, description?: string | null): void {
    if (name !== undefined) { WorkshopCategory.validateName(name); this._name = name; }
    if (description !== undefined) this._description = description;
    this._updatedAt = new Date();
  }

  get id(): WorkshopCategoryId { return this._id; }
